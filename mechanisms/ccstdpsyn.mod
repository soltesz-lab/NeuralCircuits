...
March 2017
Modified by Ivan Raikov, May 2024.
...
NEURON {
	...
	RANGE u1, u2, u3
	RANGE e, tau_open0, tau_close0, tau1, tau2, taux, A1, A2, theta1, theta2, spikewidth, wEmin, wEmax
	RANGE on, factor, trace
}
...
PARAMETER {
	e = 0	(mV)
	tau_open0 = 0.1 (ms) <1e-9,1e9>
	tau_close0 = 1.5 (ms) <1e-9,1e9>
	...
	spikewidth = 5 (ms) : how long an AP will shunt an EPSP
...
NET_RECEIVE(w, g_unit (uS), wE (uS), x, tpre (ms)) {
	...
	if (flag==0) { : Presynaptic spike
		...
		A = A + wE*g_unit*factor
		B = B + wE*g_unit*factor
                if (trace > 0) {
		...
		FOR_NETCONS(wi, g_unit_i, wEi, xi, tprei) {
