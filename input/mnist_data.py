...
def generate_inputs(
    ...
    if dataset == "train":
        imgs, lbls = load_mnist(
            path=data_prefix, digits=digits, size=train_size, dataset="train"
        )
        num_imgs = imgs.shape[0]
        imgs = imgs / 255.0
    elif dataset == "test":
        test_imgs, test_lbls = load_mnist(
            path=data_prefix, digits=digits, size=test_size, dataset="test"
        )
