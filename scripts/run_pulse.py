...
def main(args):
    ...
    if rank == 0:
        ...
        axs[0].eventplot(
            spikes, lineoffsets=np.arange(net.Ncells), orientation="horizontal"
        )
        axs[0].set(xlabel="Time (ms)", ylabel="Neuron")
        axs[1].plot(
            np.asarray(t.as_numpy()), np.asarray(soma_v.as_numpy()), label="soma"
        )
        axs[1].plot(
            np.asarray(t.as_numpy()), np.asarray(dend_v.as_numpy()), label="dend"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--run-id", type=str, action="store", required=False, help="Run id string"
    )
